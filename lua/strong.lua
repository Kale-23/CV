...
function Block(el)
    if el.t == "Para" or el.t == "Plain" then
        for k, _ in ipairs(el.content) do
            ...
            if el.content[k].t == "Str" and el.content[k].text == "Ducharme," and
                el.content[k + 1].t == "Space" and el.content[k + 2].t == "Str" and
                el.content[k + 2].text:find("^K.") then

                local _, e = el.content[k + 2].text:find("^K.")
                local rest = el.content[k + 2].text:sub(e + 1) -- empty if e+1>length
                el.content[k] = pandoc.Strong {pandoc.Str("Ducharme, K.")}
